...
<body class="h-full">
    @php
        ...
        $isSystem = request()->routeIs('admin.users*') || request()->routeIs('admin.roles*') || request()->routeIs('admin.settings*');
    @endphp
    <div x-data="{ sidebarOpen: true, openGroups: {
        ...
        <aside class="fixed inset-y-0 left-0 z-50 w-60 flex flex-col bg-gray-900 transition-all duration-300 lg:static"
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                ...
                {{-- Content Group (flat, no accordion) --}}
                <div class="space-y-0.5">
                    @if(Auth::user()->hasPermission('tours'))
                    <a href="{{ route('admin.tours') }}" wire:navigate class="flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg {{ request()->routeIs('admin.tours*') ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                        Tours
                    </a>
                    @endif
                    @if(Auth::user()->hasPermission('categories'))
                    <a href="{{ route('admin.categories') }}" wire:navigate class="flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg {{ request()->routeIs('admin.categories*') ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                    ...
                    <a href="{{ route('admin.destinations') }}" wire:navigate class="flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg {{ request()->routeIs('admin.destinations*') ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                        Destinations
                    </a>
                    @endif
                    @if(Auth::user()->hasPermission('orders'))
                    <a href="{{ route('admin.orders') }}" wire:navigate class="flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg {{ request()->routeIs('admin.orders*') ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                    ...
                    <a href="{{ route('admin.payments') }}" wire:navigate class="flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg {{ request()->routeIs('admin.payments*') ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                    ...
                    <a href="{{ route('admin.pages') }}" wire:navigate class="flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg {{ request()->routeIs('admin.pages*') ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                ...
                {{-- Appearance Group --}}
                @php
                    $hasAppearance = Auth::user()->hasPermission('media') || Auth::user()->hasPermission('sections') || Auth::user()->hasPermission('menus');
                @endphp
                @if($hasAppearance)
                <div x-data="{ open: openGroups.appearance }">
                    ...
                    <a href="{{ route('admin.reviews') }}" wire:navigate class="flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg {{ request()->routeIs('admin.reviews*') ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                    ...
                    <a href="{{ route('admin.newsletter') }}" wire:navigate class="flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-lg {{ request()->routeIs('admin.newsletter*') ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                ...
                {{-- System Group --}}
                @php
                    $hasSystem = Auth::user()->hasPermission('users') || Auth::user()->hasPermission('roles') || Auth::user()->hasPermission('permissions') || Auth::user()->hasPermission('settings') || Auth::user()->hasPermission('activity-logs');
                @endphp
                @if($hasSystem)
                <div x-data="{ open: openGroups.system }">
                    ...
                    <div x-show="open" x-collapse class="space-y-0.5 mt-0.5">
                        ...
                        <a href="{{ route('admin.roles') }}" wire:navigate class="flex items-center gap-3 px-3 py-2 pl-8 text-sm font-medium rounded-lg {{ request()->routeIs('admin.roles*') ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                        ...
                        <a href="{{ route('admin.permissions') }}" wire:navigate class="flex items-center gap-3 px-3 py-2 pl-8 text-sm font-medium rounded-lg {{ request()->routeIs('admin.permissions*') ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                            <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>
                            Permissions
                        </a>
                        @endif
                        @if(Auth::user()->hasPermission('activity-logs'))
                        <a href="{{ route('admin.activity-logs') }}" wire:navigate class="flex items-center gap-3 px-3 py-2 pl-8 text-sm font-medium rounded-lg {{ request()->routeIs('admin.activity-logs*') ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header class="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 lg:px-6 shrink-0">
                ...
                @php
                    $unreadCount = \App\Models\TourNotification::where('is_read', false)->count();
                @endphp
                <div class="flex items-center gap-2">
                    @if(Auth::user()->hasPermission('notifications'))
                    <a href="{{ route('admin.notifications') }}" wire:navigate class="relative p-2 text-gray-400 hover:text-primary-600 hover:bg-gray-100 rounded-lg transition-colors" title="Notifications">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                        @if($unreadCount > 0)
                            <span class="absolute -top-0.5 -right-0.5 w-4.5 h-4.5 flex items-center justify-center bg-red-500 text-white text-[10px] font-bold rounded-full min-w-[18px] min-h-[18px]">{{ $unreadCount > 9 ? '9+' : $unreadCount }}</span>
                        @endif
                    </a>
                    @endif
