                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($pages ?? [] as $page)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-gray-500 font-mono text-sm">/{{ $page->slug }}</td>
                            <td class="px-6 py-4">
                                @if($page->is_active)
                            ...
                            <td class="px-6 py-4 text-gray-500">{{ $page->updated_at->format('M d, Y') }}</td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.pages.edit', $page) }}" wire:navigate class="p-2 text-gray-400 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </a>
                                    <button wire:click="delete({{ $page->id }})" wire:confirm="Are you sure you want to delete this page?" class="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
