        ...
        @forelse($media ?? [] as $item)
            <div class="group relative bg-white rounded-lg border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                ...
                <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <button type="button" onclick="navigator.clipboard.writeText('{{ $item->url }}').then(() => { this.innerText = 'Copied!'; setTimeout(() => this.innerText = '', 2000); })" class="px-3 py-1.5 bg-white text-gray-800 text-xs font-medium rounded hover:bg-gray-100 transition-colors">
                        Copy URL
                    </button>
                    <button wire:click="delete({{ $item->id }})" wire:confirm="Delete this file?" class="px-3 py-1.5 bg-red-600 text-white text-xs font-medium rounded hover:bg-red-700 transition-colors">
                        Delete
                    </button>
                </div>
            </div>
