                        ...
                        <tr class="hover:bg-gray-50">
                                ...
                                @if($user->roles->isNotEmpty())
                                    <div class="flex flex-wrap gap-1">
                                        @foreach($user->roles as $role)
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $role->slug === 'administrator' ? 'bg-green-100 text-green-800' : ($role->slug === 'admin' ? 'bg-blue-100 text-blue-800' : ($role->slug === 'editor' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800')) }}">
                                                {{ $role->name }}
                                            </span>
                                        @endforeach
                                    </div>
                            ...
                            <td class="px-6 py-4">
                                @if(!$user->hasRole('administrator'))
                                    <button wire:click="toggleActive({{ $user->id }})" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border-0 cursor-pointer transition-colors
